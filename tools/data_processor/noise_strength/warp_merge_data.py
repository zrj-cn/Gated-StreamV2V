...
def load_warp_error_data(warp_error_dir):
    ...
    # 遍历noise_strength_1.warperror到noise_strength_8.warperror
    for i in range(1,10):
        file_path = os.path.join(warp_error_dir, f'noise_strength_{i}.warperror')
        if os.path.exists(file_path):
            with open(file_path, 'r') as f:
                data = json.load(f)
                warp_error_data[f'noise_strength_{i}'] = data
...
def merge_data(motion_file, eval_file, warp_error_dir, output_file):
    ...
    for i in range(1,10):
        header.append(f'noise_strength_{i}_error')
...
def main():
    ...
    output_file = '/home/zrj/project/ori_v2v/streamv2v/tools/statistics/noise_strength/warp_merged_data.csv'
